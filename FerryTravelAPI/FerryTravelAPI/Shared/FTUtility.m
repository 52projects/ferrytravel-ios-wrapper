...
+(NSDate *) convertToNSDate:(id) value {
	...
	if ([value isEqual:[NSNull null]]) {
		...
		return [FTDateUtility dateFromString:value];
	}
}
...
+(NSDate *) convertToFullNSDate: (id) value {
	if ([value isEqual:[NSNull null]]) {
		...
		return [FTDateUtility dateAndTimeFromString:value];
	}
}
...
+(NSDate *) convertToCustomNSDate: (id) value formatter: (NSString *)format {
	if ([value isEqual:[NSNull null]]) {
		...
		return [FTDateUtility dateFromString:value withDateFormat:format];
	}
}
